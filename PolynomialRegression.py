...
def showLearningCurve(model, data: np.array, target:np.array, train_ratio: float=0.7, sep: int=1):
    X_train, X_test, y_train, y_test = trainTestSplit(data, target, train_ratio=train_ratio)

    train_score = []
    test_score = []
    for i in range(sep, len(X_train) + 1, sep):
        model.fit(X_train[:i], y_train[: i])
        train_score.append(model.score(X_train[: i], y_train[: i]))
